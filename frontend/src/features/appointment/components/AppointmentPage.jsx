...
import { appointmentSections } from "../data/appointmentData";

function AppointmentPanel({ items, title }) {
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <button
            ...
            onClick={() => {
              if (item === "Appointment") {
                navigate("/appointment/entry");
              }

              if (item === "Appointment Slots") {
                navigate("/appointment/slots");
              }

              if (item === "Appointment Shift") {
                navigate("/appointment/shift-entry");
              }

              if (item === "Appointment Priority") {
                navigate("/appointment/priority-entry");
              }

              if (item === "Appointment List") {
                navigate("/appointment/bill-record");
              }
            }}
            type="button"
          >
    ...
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[360px_300px] justify-center gap-x-8 gap-y-6 max-md:grid-cols-1">
        {appointmentSections.map((section) => (
          <AppointmentPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
