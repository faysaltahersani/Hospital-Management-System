...
import { ambulanceSections } from "../data/ambulanceData";

function AmbulancePanel({ items, title }) {
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <button
            ...
            key={item}
            onClick={() => {
              if (item === "Call Ambulance Entry") {
                navigate("/ambulance/call-entry");
              }

              if (item === "Ambulance Entry") {
                navigate("/ambulance/entry");
              }

              if (item === "Call Ambulance Record") {
                navigate("/ambulance/call-record");
              }
            }}
            type="button"
          >
  ...
  return (
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[360px_300px] justify-center gap-x-8 gap-y-6 max-md:grid-cols-1">
        {ambulanceSections.map((section) => (
          <AmbulancePanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
