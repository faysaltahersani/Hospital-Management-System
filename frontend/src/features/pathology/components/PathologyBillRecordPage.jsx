...
export function PathologyBillRecordPage() {
  ...
  const loadMeta = async () => {
    ...
    try {
      const response = await apiRequest("/laboratory/pathology-bill-entry/meta");
      setMeta(response.data || emptyMeta);
    } catch (error) {
      setErrorMessage(error.message || "Failed to load pathology bill record filters");
    } finally {
  ...
  const loadOrders = async (nextPage = page) => {
    ...
    setErrorMessage("");
    try {
      const response = await apiRequest(`/laboratory/orders?${buildQueryString(nextPage)}`);
      ...
      setErrorMessage(error.message || "Failed to load pathology bill records");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const rows = useMemo(
    () =>
      orders.map((order, index) => {
        ...
        const subtotal = Number(invoice.subtotal ?? order.items?.reduce((sum, item) => sum + Number(item.price || 0), 0) ?? 0);
        ...
        return {
          ...
          updatedBy: order.creator?.full_name || order.creator?.email || "N/A",
          subTotal: subtotal,
  ...
  const printBill = async (row) => {
      ...
      .map(
        (item, index) =>
          `<tr style="border-bottom: 1px solid #eee;">
            ...
            <td style="padding: 7px 10px;">${item.test?.code || ""} - ${item.test?.name || ""}</td>
    ...
    await printHtml(
      ...
      `,
      "PATHOLOGY BILL INVOICE"
    );
  };

  const printResultReport = async (row) => {
    try {
      const response = await apiRequest(`/laboratory/orders/${row.id}/report`);
        ...
        .map(
          (item, index) =>
            `<tr style="border-bottom: 1px solid #eee;">
              ...
              <td style="padding: 6px 8px; font-weight: ${item.isGroupHeader ? 'bold' : 'normal'}">${item.test?.name || ""}</td>
              <td style="padding: 6px 8px;">${item.result_value || "—"}</td>
              <td style="padding: 6px 8px;">${item.result_notes || "—"}</td>
              <td style="padding: 6px 8px;">${item.normal_range || "—"}</td>
              <td style="padding: 6px 8px; color: ${item.status === 'Normal' ? '#388e3c' : '#d32f2f'}">${item.status || "—"}</td>
            </tr>`
        )
        .join("");
      ...
      showError("Print Failed", error.message || "Failed to load pathology report");
    }
  };

  const handleDelete = async (row) => {
    const confirmed = await confirmDelete("Delete Pathology Bill?", `Are you sure you want to delete Bill No "${row.billNo}"?`);
    if (!confirmed) return;
    setErrorMessage("");
    try {
      await apiRequest(`/laboratory/orders/${row.id}`, { method: "DELETE" });
      showSuccess("Deleted!", `Bill "${row.billNo}" has been deleted.`);
      await loadOrders(page);
    } catch (error) {
      showError("Delete Failed", error.message || "Failed to delete pathology bill");
    }
  };
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      <div className="rounded-[4px] border border-[#d3dde0] bg-white p-4 shadow-[0_10px_24px_rgba(22,36,45,0.08)]">
        <div className="mb-5 text-[16px] font-medium uppercase text-[#202f37]">Pathology Bill Record</div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex items-end">
            <button
              className="h-[36px] min-w-[220px] rounded-[3px] bg-black px-4 text-[12px] font-medium text-white disabled:cursor-not-allowed disabled:opacity-60"
              disabled={isLoading}
            ...
            {isLoading ? (
              <tr>
                <td className="border border-black px-1.5 py-4 text-center" colSpan={15}>
                  Loading pathology bill records...
                </td>
              </tr>
            ) : rows.length ? (
              rows.map((row) => (
                <Fragment key={row.id}>
                  ...
                  {recordType === "With Detail" || expandedRows.some((item) => String(item) === String(row.id)) ? (
                    <tr key={`${row.billNo}-detail`}>
                      <td className="border border-black bg-[#fafcfd] px-3 py-2" colSpan={15}>
                        <div className="space-y-1">
                          <div className="font-medium text-[#3a4951]">Bill Items</div>
                          {row.items.length ? (
                            row.items.map((item, index) => (
                              <div className="flex items-center justify-between text-[12px] text-[#4d5a61]" key={`${row.id}-${item.id || index}`}>
                                <span>
                                  {index + 1}. {item.test?.code || ""} - {item.test?.name || "Unnamed Test"}
                                </span>
                                <span>{formatMoney(item.price)}</span>
                              </div>
                            ))
                          ) : (
                            <div className="text-[#7b878d]">No test items found.</div>
                          )}
                        </div>
                      </td>
                    </tr>
              ...
              <tr>
                <td className="border border-black px-1.5 py-4 text-center" colSpan={15}>
                  No pathology bill records found.
                </td>
              </tr>
            )}
            <tr>
