...
export function PathologyTestListPage() {
  ...
  const loadMeta = async () => {
    ...
    try {
      const response = await apiRequest("/laboratory/pathology-test-entry/meta");
      setMeta(response.data || emptyMeta);
    } catch (error) {
      setErrorMessage(error.message || "Failed to load pathology test metadata");
    } finally {
  ...
  const loadTests = async (nextPage = page) => {
    ...
    setErrorMessage("");
    try {
      const response = await apiRequest(`/laboratory/tests?${buildQueryString(nextPage)}`);
      ...
      setErrorMessage(error.message || "Failed to load pathology tests");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const rows = useMemo(
    () =>
      tests.map((test, index) => ({
        ...
        method: test.method || "",
        baseCharge: Number(test.base_charge || 0),
        finalCharge: Number(test.final_charge || 0),
        createdBy: formatDateLabel(test.createdAt || test.created_at) || "N/A",
        updatedBy: formatDateLabel(test.updatedAt || test.updated_at) || "N/A",
        reportDeliveryDay: test.report_delivery_day || "",
        taxRate: test.tax_rate ?? "",
  ...
  const handleDelete = async (row) => {
    const confirmed = window.confirm(`Delete pathology test "${row.testName}"?`);
    if (!confirmed) return;
    setErrorMessage("");
    try {
      await apiRequest(`/laboratory/tests/${row.id}`, { method: "DELETE" });
      await loadTests(page);
    } catch (error) {
      setErrorMessage(error.message || "Failed to delete pathology test");
    }
  };
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      <div className="rounded-[4px] border border-[#d3dde0] bg-white p-4 shadow-[0_10px_24px_rgba(22,36,45,0.08)]">
        <div className="mb-5 text-[16px] font-medium uppercase text-[#202f37]">Pathology Test</div>
              ...
              rows.map((row) => (
                <Fragment key={row.id}>
                  ...
                  {recordType === "With Detail" || expandedRows.includes(row.id) ? (
                    <tr>
                      <td className="border border-black bg-[#fafcfd] px-3 py-2" colSpan={11}>
                        <div className="grid grid-cols-2 gap-3 text-[12px] text-[#4d5a61] max-md:grid-cols-1">
                          <div>
                            <span className="font-medium">Report Delivery Day:</span> {row.reportDeliveryDay || "N/A"}
                          </div>
                          <div>
                            <span className="font-medium">Tax Rate:</span> {row.taxRate === "" ? "N/A" : row.taxRate}
                          </div>
                          <div className="col-span-2">
                            <span className="font-medium">Parameters:</span>{" "}
                            {row.parameters.length ? row.parameters.join(", ") : "No parameters linked"}
                          </div>
                        </div>
                      </td>
                    </tr>
