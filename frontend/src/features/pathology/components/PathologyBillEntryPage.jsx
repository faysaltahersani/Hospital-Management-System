...
export function PathologyBillEntryPage() {
    ...
    const loadMeta = async () => {
      ...
      try {
        const [metaRes, patientsRes, doctorsRes, testsRes] = await Promise.all([
          apiRequest("/laboratory/pathology-bill-entry/meta").catch(() => null),
          apiRequest("/patients?limit=100").catch(() => ({ data: [] })),
          apiRequest("/doctors?limit=100").catch(() => ({ data: [] })),
          apiRequest("/laboratory/tests?limit=100").catch(() => ({ data: [] })),
        ]);

        if (!isMounted) return;

        const payload = metaRes?.data || {};
        const patientList = payload.patients?.length ? payload.patients : (patientsRes.data || []);
        const doctorList = payload.doctors?.length ? payload.doctors : (doctorsRes.data || []);
        const testList = payload.tests?.length ? payload.tests : (testsRes.data || []);

        const fullMeta = {
          next_bill_no: payload.next_bill_no || `BILL-${Date.now().toString().slice(-6)}`,
          next_patient_code: payload.next_patient_code || `PAT-${Date.now().toString().slice(-5)}`,
          accounts: payload.accounts || ["Cash", "Card", "Bkash", "Nagad"],
        ...
        setMeta(fullMeta);
        setBillNo(fullMeta.next_bill_no);
        setSelectedAccount(fullMeta.accounts[0] || "");
        resetNewPatientForm(fullMeta.next_patient_code);
      } catch (error) {
        if (!isMounted) return;
        setErrorMessage(error.message || "Failed to load pathology bill form data");
      } finally {
  ...
  const addTest = () => {
    ...
    if (items.some((item) => item.test_id === selectedTest.id)) {
      setErrorMessage("This test has already been added.");
      return;
    }

    const price = Number(selectedTestPrice || selectedTest.base_charge || selectedTest.price || 0);
    const taxRate = Number(selectedTest.tax_rate || 0);
  ...
  const addPayment = () => {
    ...
    if (total > 0 && totalPaid + amount > total) {
      setErrorMessage("Payment amount cannot exceed the pathology bill total.");
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (!items.length) {
      setErrorMessage("Add at least one pathology test before saving.");
      return;
    }

    setIsSaving(true);
    try {
      const response = await apiRequest("/laboratory/pathology-bills", {
        method: "POST",
        body: JSON.stringify({
      ...
      setSuccessMessage(`Pathology bill ${bill.order_code} saved successfully.`);

      const metaResponse = await apiRequest("/laboratory/pathology-bill-entry/meta");
      const nextMeta = metaResponse.data || emptyMeta;
      setMeta(nextMeta);
      resetForm(nextMeta.next_bill_no);
      setSelectedAccount(nextMeta.accounts?.[0] || "");
    } catch (error) {
      setErrorMessage(error.message || "Failed to save pathology bill");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSavePatient = async () => {
    ...
    try {
      ...
      setMeta(nextMeta);
      setPatientId(String(createdPatient.id));
      setSelectedAccount((current) => current || nextMeta.accounts?.[0] || "");
      resetNewPatientForm(nextMeta.next_patient_code || "");
    } catch (error) {
    ...
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      <div className="rounded-[8px] border border-[#d3dde0] bg-white p-5 shadow-[0_10px_24px_rgba(22,36,45,0.08)]">
        ...
        <div className="mb-4 rounded-[6px] border border-[#d2dde0] p-3">
          ...
          <div className="mb-3 grid grid-cols-[60px_1.7fr_0.8fr_0.7fr_0.7fr_0.8fr_0.7fr] gap-3 max-xl:grid-cols-2 max-md:grid-cols-1">
            <div />
            ...
            <label className="text-[11px] text-[#7c8a92]">
              ...
              <input
                ...
                readOnly
                type="text"
                value={formatMoney((Number(selectedTestPrice || 0) * selectedTestTaxRate) / 100)}
              />
            </label>
            <label className="text-[11px] text-[#7c8a92]">
              ...
              <input
                ...
                readOnly
                type="text"
                value={formatMoney(Number(selectedTestPrice || 0) + (Number(selectedTestPrice || 0) * selectedTestTaxRate) / 100)}
              />
            </label>
          ...
          <div className="grid grid-cols-[60px_1.7fr_0.8fr_0.7fr_0.7fr_0.8fr_0.7fr] border-t border-[#dbe4e7] bg-[#e8f5f6] px-3 py-2 text-[12px] text-[#4d5a61] max-lg:hidden">
            <span>SL</span>
          ...
          {items.length ? (
            items.map((item, index) => (
              <div
                className="grid grid-cols-[60px_1.7fr_0.8fr_0.7fr_0.7fr_0.8fr_0.7fr] border-t border-[#eef2f4] px-3 py-2 text-[12px] text-[#34434a] max-lg:grid-cols-1"
                key={`${item.test_id}-${index}`}
              >
                <span>{index + 1}</span>
                <span>{item.name}</span>
                <span>{formatMoney(item.price)}</span>
                <span>{Number(item.tax_rate || 0)}%</span>
                <span>{formatMoney(item.tax_amount)}</span>
                <span>{formatMoney(Number(item.price || 0) + Number(item.tax_amount || 0))}</span>
            ...
            <div className="border-t border-[#eef2f4] px-3 py-3 text-[12px] text-[#7b878d]">No tests added yet.</div>
          )}
        </div>
          ...
          <div className="rounded-[6px] border border-[#d2dde0] p-3">
            ...
            <div className="space-y-3">
              <select
                ...
                value={doctorId}
              >
                <option value="">Referral Doctor</option>
