...
export function SupplierLedgerReportPage() {
  const [suppliers, setSuppliers] = useState([{ value: "", label: "SELECT SUPPLIER" }]);
  ...
  const loadSuppliers = async () => {
    ...
    if (!selectedSupplier && items[0]?.id) {
      setSelectedSupplier(String(items[0].id));
    }
  };

  const loadReport = async (supplierId = selectedSupplier, from = fromDate, to = toDate) => {
    if (!supplierId) {
    ...
    try {
      ...
      const response = await apiRequest(`/reports/supplier-ledger?${params.toString()}`);
      setReport(
        response.data || {
          supplier: null,
          opening_balance: 0,
      ...
      setErrorMessage(error.message || "Failed to load supplier ledger.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (selectedSupplier) {
      loadReport(selectedSupplier, fromDate, toDate);
    }
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      ...
      <div className="rounded-[4px] border border-[#d9e1e5] bg-white px-4 py-5 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="mb-5 text-[18px] font-medium text-[#1f2c33]">SUPPLIER LEDGER</div>
      ...
      <div className="mt-2 rounded-[2px] border border-[#d9e1e5] bg-white px-3 py-3 shadow-[0_2px_8px_rgba(22,36,45,0.06)]">
        <div className="flex items-center gap-5 text-[#2d2d2d]">
          ...
          <button
            disabled={!report.supplier}
            onClick={() => downloadCsv(`supplier-ledger-${report.supplier?.supplier_code || "report"}.csv`, csvRows)}
            type="button"
          >
            <svg aria-hidden="true" className="h-[22px] w-[22px]" fill="currentColor" viewBox="0 0 24 24">
      ...
      {report.supplier ? (
        <div className="mt-4 rounded-[4px] border border-[#d9e1e5] bg-white px-4 py-3 text-[13px] text-[#21343e] shadow-[0_2px_8px_rgba(22,36,45,0.06)]">
          <div><strong>Supplier:</strong> {report.supplier.name}</div>
          <div><strong>Code:</strong> {report.supplier.supplier_code || "-"}</div>
        ...
        <table className="min-w-full border-collapse text-left text-[12px] text-[#1f2c33]">
          ...
          <tbody>
            ...
            {isLoading ? (
              ...
              <tr>
                <td className="border border-[#1f2c33] px-2 py-8 text-center text-[#5f6b76]" colSpan="8">
                  No ledger entries found for the selected supplier and date range.
                </td>
              </tr>
            )}
