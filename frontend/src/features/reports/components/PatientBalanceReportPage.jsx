...
const FILTER_TYPES = ["All", "By Patient", "By Date"];
...
export function PatientBalanceReportPage() {
  ...
  const loadRows = async (targetPage = page) => {
    ...
    try {
      ...
      const response = await apiRequest(`/reports/patient-balance?${params.toString()}`);
      ...
      setTotalPages(1);
      setErrorMessage(error.message || "Failed to load patient balance report.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    Promise.all([loadPatients(), loadRows(1)]).catch(() => {});
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      ...
      <div className="rounded-[4px] border border-[#d9e1e5] bg-white px-4 py-5 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="mb-6 text-[18px] font-medium text-[#1f2c33]">PATIENT BALANCE REPORT</div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-5">
          ...
          {filterType === "By Patient" ? (
            <SelectField label="PATIENT" onChange={(event) => setSelectedPatient(event.target.value)} options={patientOptions} value={selectedPatient} />
          ) : null}
          {filterType === "By Date" ? (
            <>
      ...
      <div className="mt-2 rounded-[2px] border border-[#d9e1e5] bg-white px-3 py-4 shadow-[0_2px_8px_rgba(22,36,45,0.06)]">
        <div className="flex items-center gap-5 text-[#2d2d2d]">
          ...
          <button onClick={() => downloadCsv("patient-balance-report.csv", csvRows)} type="button">
            <svg aria-hidden="true" className="h-[22px] w-[22px]" fill="currentColor" viewBox="0 0 24 24">
