...
export function OpdBillRecordReportPage() {
  ...
  const loadRows = async (targetPage = 1) => {
    ...
    try {
      ...
      const response = await apiRequest(`/opd?${params.toString()}`);
      setRows(response.data || []);
      setMeta(response.meta?.pagination || { page: targetPage, limit: 10, total_pages: 1 });
      setPage(targetPage);
    } catch (error) {
      setRows([]);
      setErrorMessage(error.message || "Failed to load OPD bill record.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    Promise.all([loadLists(), loadRows(1)]).catch((error) => {
      setErrorMessage(error.message || "Failed to load OPD bill report.");
    });
  ...
  const totals = useMemo(
    () =>
      rows.reduce(
        (sum, row) => {
          const amount = Number(row.invoice?.total ?? row.consultation_fee ?? 0);
          const paid = Number(row.invoice?.paid_amount ?? 0);
          return {
            amount: sum.amount + amount,
            paid: sum.paid + paid,
            due: sum.due + (amount - paid),
          };
        },
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      ...
      <div className="rounded-[4px] border border-[#d9e1e5] bg-white px-4 py-5 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="mb-5 text-[18px] font-medium text-[#1f2c33]">OPD BILL RECORD</div>
        ...
        <table className="min-w-full border-collapse text-left text-[12px] text-[#1f2c33]">
          ...
          <tbody>
            {isLoading ? (
              ...
              rows.map((row, index) => {
                ...
                return (
                  <tr key={row.id}>
                    ...
                    <td className="border border-[#1f2c33] px-2 py-1.5">{row.visit_code || row.id}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5">{row.patient?.full_name || "-"}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5">{formatDate(row.visit_date)}</td>
              ...
              <tr>
                <td className="border border-[#1f2c33] px-2 py-8 text-center text-[#5f6b76]" colSpan="9">
                  No OPD bills found for the selected filters.
                </td>
              </tr>
            )}
