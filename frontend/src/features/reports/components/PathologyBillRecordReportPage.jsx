...
export function PathologyBillRecordReportPage() {
  ...
  const loadRows = async (targetPage = 1) => {
    ...
    try {
      ...
      const response = await apiRequest(`/laboratory/orders?${params.toString()}`);
      setRows(response.data || []);
      setMeta(response.meta?.pagination || { page: targetPage, limit: 10, total_pages: 1 });
      setPage(targetPage);
    } catch (error) {
      setRows([]);
      setErrorMessage(error.message || "Failed to load pathology bill record.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    Promise.all([loadLists(), loadRows(1)]).catch((error) => {
      setErrorMessage(error.message || "Failed to load pathology bill report.");
    });
  ...
  return (
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      ...
      <div className="rounded-[4px] border border-[#d9e1e5] bg-white px-4 py-5 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="mb-5 text-[18px] font-medium text-[#1f2c33]">PATHOLOGY BILL RECORD</div>
            ...
            {isLoading ? (
              <tr>
                <td className="border border-[#1f2c33] px-2 py-8 text-center text-[#5f6b76]" colSpan="9">
                  Loading pathology bills...
                </td>
              </tr>
            ) : rows.length ? (
              rows.map((row, index) => {
                const amount = Number(row.invoice?.total ?? row.total ?? 0);
                const paid = Number(row.invoice?.paid_amount ?? 0);
                return (
                  <tr key={row.id}>
                    <td className="border border-[#1f2c33] px-2 py-1.5">{(page - 1) * 10 + index + 1}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5">{row.order_code || row.id}</td>
                    ...
                    <td className="border border-[#1f2c33] px-2 py-1.5">{formatDate(row.ordered_at || row.createdAt)}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5">{row.invoice?.status || row.status || "-"}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5 text-right">{money(amount)}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5 text-right">{money(paid)}</td>
                    <td className="border border-[#1f2c33] px-2 py-1.5 text-right">{money(amount - paid)}</td>
                  </tr>
                );
              })
            ) : (
              <tr>
                <td className="border border-[#1f2c33] px-2 py-8 text-center text-[#5f6b76]" colSpan="9">
                  No pathology bills found for the selected filters.
                </td>
              </tr>
            )}
