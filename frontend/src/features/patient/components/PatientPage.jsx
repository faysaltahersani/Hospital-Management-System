...
import { patientSections } from "../data/patientData";
import { patientRoutes } from "../data/patientRoutes";

function PatientPanel({ items, title }) {
    ...
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[310px_310px] justify-center gap-x-4 gap-y-6 max-md:grid-cols-1">
        {patientSections.map((section) => (
          <PatientPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
