...
const UNIT_TYPE = "radiology_test_unit";
...
export function RadiologyTestUnitEntryPage() {
  ...
  const loadUnits = async () => {
    ...
    setErrorMessage("");
    try {
      const response = await apiRequest(`/settings/master-options?type=${UNIT_TYPE}&limit=100`);
      setUnits(response.data || []);
    } catch (error) {
      setErrorMessage(error.message || "Failed to load radiology test units");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredUnits = useMemo(() => {
    ...
    return units.filter((item) => (item.label || "").toLowerCase().includes(query));
  }, [search, units]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrorMessage(error.message || "Failed to save radiology test unit");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleEdit = (unit) => {
    ...
    setForm({ name: unit.label || "" });
  };

  const handleDelete = async (unit) => {
    const confirmed = window.confirm(`Delete unit "${unit.label}"?`);
    if (!confirmed) return;

    setErrorMessage("");
    setSuccessMessage("");
    try {
      await apiRequest(`/settings/master-options/${unit.id}`, {
        method: "DELETE",
      });
      if (editingId === unit.id) resetForm();
      setSuccessMessage(`Unit ${unit.label} deleted successfully.`);
      await loadUnits();
    } catch (error) {
      setErrorMessage(error.message || "Failed to delete radiology test unit");
    }
  };

  const exportCsv = () => {
    const rows = [["SL", "NAME"], ...filteredUnits.map((item, index) => [index + 1, item.label || ""])];
    const csv = rows.map((row) => row.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "radiology-test-units.csv";
    link.click();
        ...
        <div className="grid grid-cols-[1fr_1fr] gap-5 max-lg:grid-cols-1">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <button
                className="min-w-[275px] flex-1 rounded-[6px] border border-[#8eb6ff] bg-white px-4 py-3 text-[15px] font-semibold text-[#159020] disabled:cursor-not-allowed disabled:opacity-60"
                disabled={isSaving}
                onClick={handleSave}
                type="button"
              >
              ...
              <button
                className="min-w-[275px] flex-1 rounded-[6px] border border-[#ff8f8f] bg-white px-4 py-3 text-[15px] font-semibold text-[#ff3f34]"
                onClick={() => {
                  setErrorMessage("");
                  setSuccessMessage("");
                  resetForm();
          ...
          <div>
            ...
            <div className="overflow-hidden rounded-[6px] border border-[#d7dde2] bg-white shadow-[0_10px_24px_rgba(22,36,45,0.08)]">
              ...
              {isLoading ? (
                <div className="px-4 py-6 text-[14px] text-[#61727a]">Loading units...</div>
                ...
                <div className="px-4 py-6 text-[14px] text-[#61727a]">No units found.</div>
              )}

              <div className="flex items-center justify-end gap-6 px-4 py-4 text-[15px] text-[#4b5961]">
                <span>Rows per page 100</span>
                <span>{filteredUnits.length ? `1-${filteredUnits.length} of ${filteredUnits.length}` : "0 of 0"}</span>
              </div>
            </div>
          </div>
        </div>
