...
export function RadiologyParameterEntryPage() {
  ...
  const loadPageData = async () => {
    ...
    try {
      const [unitsResponse, listResponse] = await Promise.all([
        apiRequest("/settings/master-options?type=radiology_test_unit&limit=100").catch(() => ({ data: [] })),
      ...
      if (!fetchedUnits.length) {
        fetchedUnits = [
          { id: 1, label: "mm", code: "mm" },
          { id: 2, label: "cm", code: "cm" },
          { id: 3, label: "HU", code: "hu" },
          { id: 4, label: "mGy", code: "mgy" },
          { id: 5, label: "mSv", code: "msv" },
          { id: 6, label: "dB", code: "db" },
          { id: 7, label: "MHz", code: "mhz" },
          { id: 8, label: "SUV", code: "suv" },
          { id: 9, label: "mm²", code: "mm2" },
          { id: 10, label: "cm³", code: "cm3" },
        ];
      }
      ...
      setErrorMessage(error.message || "Failed to load radiology parameters");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await apiRequest(`/radiology/parameters/${editingId}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        });
        setSuccessMessage(`Parameter ${payload.name} updated successfully.`);
      } else {
        await apiRequest("/radiology/parameters", {
          method: "POST",
          body: JSON.stringify(payload),
        });
      ...
      setErrorMessage(error.message || "Failed to save radiology parameter");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (parameter) => {
    ...
    setSuccessMessage("");
    try {
      await apiRequest(`/radiology/parameters/${parameter.id}`, {
        method: "DELETE",
      });
      ...
      setErrorMessage(error.message || "Failed to delete radiology parameter");
    }
  };
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = "radiology-parameters.csv";
    link.click();
