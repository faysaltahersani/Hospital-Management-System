...
import { hrPayrollSections } from "../data/hrPayrollData";
import { hrPayrollRoutes } from "../data/hrPayrollRoutes";

function HrPayrollPanel({ items, title }) {
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <button
            ...
            key={item}
            onClick={() => {
              const path = hrPayrollRoutes[item];
  ...
  return (
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[310px_310px] justify-center gap-x-4 gap-y-6 max-md:grid-cols-1">
        {hrPayrollSections.map((section) => (
          <HrPayrollPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
