...
import { opdSections } from "../data/opdData";

function OpdPanel({ items, title }) {
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <button
            ...
            key={item}
            onClick={() => {
              if (item === "OPD Bill Entry") {
                navigate("/opd/bill-entry");
              }

              if (item === "Symptoms Manage") {
                navigate("/opd/symptoms-manage");
              }

              if (item === "OPD Bill Record") {
                navigate("/opd/bill-record");
              }
            }}
            type="button"
          >
    ...
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[360px_300px] justify-center gap-x-8 gap-y-6 max-md:grid-cols-1">
        {opdSections.map((section) => (
          <OpdPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
