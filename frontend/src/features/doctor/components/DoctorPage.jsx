...
import { doctorSections } from "../data/doctorData";
import { doctorRoutes } from "../data/doctorRoutes";

function DoctorPanel({ items, title }) {
...
export function DoctorPage() {
  return (
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[360px] justify-center gap-x-8 gap-y-6 max-md:grid-cols-1">
        {doctorSections.map((section) => (
          <DoctorPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
