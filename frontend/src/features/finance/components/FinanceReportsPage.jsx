import { financeReportsSections } from "../data/financeReportsData";

function FinanceReportsPanel({ items, title }) {
  return (
...
export function FinanceReportsPage() {
  return (
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[1180px] grid-cols-3 justify-center gap-x-8 gap-y-6 max-lg:grid-cols-2 max-md:grid-cols-1">
        {financeReportsSections.map((section) => (
          <FinanceReportsPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
