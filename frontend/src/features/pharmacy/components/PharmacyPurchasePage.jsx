...
export function PharmacyPurchasePage() {
  ...
  const enrichedRows = useMemo(
    () =>
      rows.map((row) => {
        const quantity = toMoney(row.quantity || 0);
        const purchasePrice = toMoney(row.purchase_price || 0);
        const rowTaxRate = toMoney(row.tax_rate || 0);
        const baseTotal = quantity * purchasePrice;
  ...
  const subtotal = toMoney(enrichedRows.reduce((sum, row) => sum + Number(row.total || 0), 0));
  const discountAmount = toMoney((subtotal * Number(form.discount_percent || 0)) / 100);
  const afterDiscount = toMoney(Math.max(subtotal - discountAmount, 0));
  const taxAmount = toMoney((afterDiscount * Number(form.tax_rate || 0)) / 100);
  const totalAmount = toMoney(Math.max(afterDiscount + taxAmount, 0));
  const totalPaid = toMoney(payments.reduce((sum, payment) => sum + Number(payment.amount || 0), 0));
  const dueAmount = toMoney(Math.max(totalAmount - totalPaid, 0));
  ...
  const loadPageData = async () => {
    ...
    try {
      const [suppliersResponse, medicinesResponse, metaResponse, accountsResponse] = await Promise.all([
        ...
        apiRequest("/pharmacy/purchases/meta"),
        apiRequest("/billing/accounts?limit=100"),
      ]);

      const nextCode = metaResponse.data?.next_purchase_code || "";
      ...
      setErrorMessage(error.message || "Failed to load pharmacy purchase form data");
      return "";
    } finally {
      setIsLoading(false);
    }
      ...
      current.map((row) => {
        ...
        if (field === "medicine_id") {
          ...
          return {
            ...row,
            medicine_id: value,
            purchase_price: medicine ? String(medicine.purchase_price || "") : row.purchase_price,
            sale_price: medicine ? String(medicine.sale_price || "") : row.sale_price,
  ...
  const addPayment = () => {
    ...
    const amount = toMoney(form.payment_amount || 0);
    ...
    if (toMoney(totalPaid + amount) > totalAmount) {
      setErrorMessage("Payment amount cannot exceed the purchase total.");
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (invalidRow) {
      setErrorMessage("Each medicine row requires medicine, quantity, purchase price, and sale price.");
      return;
    }

    setIsSaving(true);
    try {
      const response = await apiRequest("/pharmacy/purchases", {
        method: "POST",
        body: JSON.stringify({
      ...
      setSuccessMessage(`Pharmacy purchase ${response.data?.purchase_code || ""} saved successfully.`);
    } catch (error) {
      setErrorMessage(error.message || "Failed to save pharmacy purchase");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      <div className="rounded-[5px] border border-[#d6e2e7] bg-white p-4 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="space-y-4">
          ...
          <div className="rounded-[4px] bg-[#38439c] px-4 py-2 text-white">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-[15px] font-semibold">Pharmacy Purchase Entry</div>
                <div className="text-[10px] text-white/85">Supplier wise medicine purchase & payment tracking</div>
              </div>
              <div className="pt-2 text-[11px] font-semibold">{selectedSupplier ? selectedSupplier.name : "No supplier selected"}</div>
            </div>
          </div>

          <div className="grid grid-cols-[220px_210px_minmax(260px,1fr)_220px] items-start gap-3 max-lg:grid-cols-2 max-md:grid-cols-1">
            <Frame label="Bill No">
              <TextInput readOnly value={form.purchase_code} />
            </Frame>
            <Frame label="Created Date">
              <TextInput onChange={(event) => updateForm("purchased_at", event.target.value)} type="date" value={form.purchased_at} />
            </Frame>
            ...
            <div className="space-y-1 pt-1 text-[11px] text-[#59656d]">
              <div>Name: {selectedSupplier?.name || "-"}</div>
              <div>Code: {selectedSupplier?.supplier_code || "-"}</div>
              <div>Contact: {selectedSupplier?.phone || "-"}</div>
              <div>Address: {selectedSupplier?.address || "-"}</div>
            </div>
          </div>

          <div className="rounded-[4px] border border-[#cde0e7] p-3">
            <div className="mb-3 border-l-2 border-[#0d7267] pl-2 text-[13px] font-semibold text-[#1f2c33]">Add Medicines</div>

            <div className="mt-4 overflow-hidden rounded-[3px] border border-[#dbe6eb]">
              <div className="grid grid-cols-[50px_minmax(170px,1.9fr)_100px_80px_100px_100px_120px_100px_90px_90px_70px] items-center gap-3 bg-[#e7f4f4] px-3 py-2 text-[12px] text-[#2a3940] max-lg:hidden">
                <span>SL</span>
                <span>Medicine</span>
              ...
              {enrichedRows.length ? (
                enrichedRows.map((row, index) => (
                  <div
                    className="grid grid-cols-[50px_minmax(170px,1.9fr)_100px_80px_100px_100px_120px_100px_90px_90px_70px] items-center gap-3 border-t border-[#dbe6eb] px-3 py-3 max-lg:grid-cols-1"
                    key={row.id}
                  >
                    ...
                    <Frame>
                      <TextInput onChange={(event) => updateRow(row.id, "purchase_price", event.target.value)} value={row.purchase_price} />
                    </Frame>
                    <Frame>
                      <TextInput onChange={(event) => updateRow(row.id, "quantity", event.target.value)} type="number" value={row.quantity} />
                    </Frame>
                    <Frame>
                      <TextInput onChange={(event) => updateRow(row.id, "sale_price", event.target.value)} value={row.sale_price} />
                    </Frame>
                    <Frame>
            ...
            <button
              className="mt-3 rounded-[3px] border border-[#80b2f2] px-4 py-2 text-[12px] font-semibold text-[#1f73de]"
          ...
          <div className="grid grid-cols-[minmax(250px,1fr)_minmax(420px,2fr)] gap-3 max-lg:grid-cols-1">
            <div className="rounded-[4px] border border-[#cde0e7] p-3">
              <div className="mb-3 border-l-2 border-[#0d7267] pl-2 text-[13px] font-semibold text-[#1f2c33]">Note</div>
              <Frame>
            ...
            <div className="rounded-[4px] border border-[#cde0e7] p-3">
              <div className="mb-3 border-l-2 border-[#0d7267] pl-2 text-[13px] font-semibold text-[#1f2c33]">Billing Summary</div>
          ...
          <div className="ml-auto max-w-[820px] rounded-[4px] border border-[#cde0e7]">
            ...
            <div className="p-3">
              <div className="grid grid-cols-[minmax(200px,1.4fr)_160px_84px] gap-3 max-md:grid-cols-1">
                <Frame>
                  <SelectInput
                ...
                <button className="h-[32px] self-start rounded-[3px] bg-[#b4beb4] text-[12px] font-semibold text-[#182126]" onClick={addPayment} type="button">
                  ADD
                </button>
              </div>
