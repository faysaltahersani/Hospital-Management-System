...
export function PharmacySalesReturnPage() {
  ...
  const loadPageData = async () => {
    ...
    try {
      const [patientsResponse, doctorsResponse, medicinesResponse, metaResponse, accountsResponse] = await Promise.all([
        apiRequest("/patients?limit=100"),
        apiRequest("/doctors?limit=100"),
        apiRequest("/pharmacy/medicines?limit=100"),
        apiRequest("/pharmacy/sales-returns/meta"),
      ...
      setErrorMessage(error.message || "Failed to load pharmacy sales return form data");
      return "";
    } finally {
      setIsLoading(false);
    }
      ...
      current.map((row) => {
        ...
        if (field === "medicine_id") {
          ...
          return {
            ...row,
            medicine_id: value,
            return_price: medicine ? String(medicine.sale_price || "") : row.return_price,
  ...
  const addPayment = () => {
    ...
    if (totalPaid + amount > totalAmount) {
      setErrorMessage("Payment amount cannot exceed the sales return total.");
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (!form.patient_id) {
      setErrorMessage("Select a patient before saving.");
      return;
    }
    if (!rows.length) {
    ...
    setIsSaving(true);
    try {
      const response = await apiRequest("/pharmacy/sales-returns", {
        method: "POST",
        body: JSON.stringify({
      ...
      const msg = `Pharmacy sales return ${response.data?.return_code || ""} saved successfully.`;
      setSuccessMessage(msg);
      showSuccess("Success!", msg);
    } catch (error) {
      const err = error.message || "Failed to save pharmacy sales return";
      setErrorMessage(err);
      showError("Error!", err);
    } finally {
      setIsSaving(false);
    ...
    <section className="mx-auto max-w-[1280px] pt-[2px]">
      <div className="rounded-[5px] border border-[#d6e2e7] bg-white p-4 shadow-[0_8px_22px_rgba(22,36,45,0.08)]">
        <div className="space-y-4">
          ...
          <div className="rounded-[3px] bg-[#dff3f4] px-3 py-2 text-[15px] text-[#223039]">PHARMACY SALES RETURN ENTRY</div>
          ...
          <div className="grid grid-cols-[180px_180px_170px_minmax(260px,1fr)] items-start gap-3 max-lg:grid-cols-2 max-md:grid-cols-1">
            ...
            <div className="relative">
              <Frame label="Patient">
                <SelectInput
                  ...
                  placeholder={isLoading ? "Loading patients..." : "Search Patient"}
            ...
            <div className="rounded-[3px] bg-[#f2f2f2] px-4 py-3 text-[13px] text-[#1f2c33]">
              <span className="mr-6">Patient Name: {selectedPatient?.full_name || "-"}</span>
              <span className="mr-6">Patient Code: {selectedPatient?.patient_code || "-"}</span>
              <span className="mr-6">Contact No: {selectedPatient?.phone || "-"}</span>
          ...
          <div>
            ...
            <div className="mt-4 overflow-hidden rounded-[3px] border border-[#dbe6eb]">
              <div className="grid grid-cols-[50px_minmax(170px,1.8fr)_110px_80px_110px_120px_100px_90px_90px_70px] bg-[#e7f4f4] px-3 py-2 text-[12px] text-[#2a3940] max-lg:hidden">
                <span>SL</span>
                <span>Medicine</span>
