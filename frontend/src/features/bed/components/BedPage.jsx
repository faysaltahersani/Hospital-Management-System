...
import { bedSections } from "../data/bedData";
import { bedRoutes } from "../data/bedRoutes";

function BedPanel({ items, title }) {
    ...
    <section className="mx-auto max-w-[1240px] pt-[2px]">
      ...
      <div className="mx-auto grid max-w-[860px] grid-cols-[360px] justify-center gap-x-8 gap-y-6 max-md:grid-cols-1">
        {bedSections.map((section) => (
          <BedPanel key={section.title} {...section} />
        ))}
      </div>
    </section>
  );
