...
function checkPageSpace(requiredSpace = 50) {
...
function renderHeaderBanner() {
  doc.rect(40, 40, 515, 65).fill(C_PRIMARY);
  doc.fillColor('#FFFFFF').fontSize(19).font('Helvetica-Bold').text('HOSPITAL MANAGEMENT SYSTEM (HMS)', 55, 52);
  doc.fillColor('#E0F2F1').fontSize(10.5).font('Helvetica').text('Comprehensive Enterprise System Architecture & Feature Documentation', 55, 76);
  doc.fillColor('#B2DFDB').fontSize(8.5).font('Helvetica').text('DeshIT-BD Software Solutions  |  Version 2.0  |  August 2026', 55, 90);
  doc.y = 120;
}
...
function renderSectionHeader(numberStr, titleStr) {
  checkPageSpace(45);
  doc.moveDown(0.4);
  doc.rect(40, doc.y, 515, 22).fill(C_SECONDARY);
  doc.fillColor('#FFFFFF').fontSize(11).font('Helvetica-Bold').text(`${numberStr}. ${titleStr.toUpperCase()}`, 48, doc.y + 5);
  doc.moveDown(0.6);
...
function renderSubSection(titleStr) {
  checkPageSpace(35);
  doc.fillColor(C_ACCENT).fontSize(10).font('Helvetica-Bold').text(titleStr);
  doc.moveDown(0.2);
  doc.fillColor(C_TEXT);
}
...
function renderParagraph(textStr) {
  checkPageSpace(25);
  doc.font('Helvetica').fontSize(9).fillColor(C_TEXT).text(textStr, { align: 'justify', lineGap: 2 });
  doc.moveDown(0.3);
}

function renderBullet(keyStr, valStr) {
  checkPageSpace(18);
  doc.font('Helvetica-Bold').fontSize(9).fillColor(C_PRIMARY).text(`- ${keyStr}: `, { continued: true });
  doc.font('Helvetica').fontSize(9).fillColor(C_TEXT).text(valStr, { align: 'justify', lineGap: 2 });
  doc.moveDown(0.2);
}

function renderFeatureBox(boxTitle, bulletPoints) {
  const boxHeight = bulletPoints.length * 15 + 20;
  checkPageSpace(boxHeight + 10);
  const startY = doc.y;
  doc.rect(40, startY, 515, boxHeight).fillAndStroke(C_BOX_BG, C_BORDER);
  doc.fillColor(C_SECONDARY).fontSize(9.5).font('Helvetica-Bold').text(boxTitle, 50, startY + 5);
  let curY = startY + 18;
  bulletPoints.forEach((pt) => {
    doc.fillColor(C_PRIMARY).fontSize(8.5).font('Helvetica-Bold').text(`> `, 52, curY, { continued: true, lineBreak: false });
    doc.fillColor(C_TEXT).fontSize(8.5).font('Helvetica-Bold').text(`${pt.label}: `, { continued: true, lineBreak: false });
    doc.fillColor(C_MUTED).fontSize(8.5).font('Helvetica').text(pt.text, { lineBreak: false });
    curY += 15;
  });
...
checkPageSpace(70);
const boxY = doc.y;
doc.rect(40, boxY, 515, 55).fillAndStroke('#E0F2F1', C_PRIMARY);
doc.fillColor(C_SECONDARY).fontSize(10.5).font('Helvetica-Bold').text('OFFICIAL PROJECT SIGN-OFF', 55, boxY + 10);
doc.fillColor(C_TEXT).fontSize(9).font('Helvetica').text('Prepared & Certified by: DeshIT-BD Software Development Team', 55, boxY + 25);
doc.fillColor(C_MUTED).fontSize(8.5).font('Helvetica').text('Date of Certification: 09 August 2026  |  Status: Production Approved', 55, boxY + 38);

// --- FOOTER & PAGE NUMBERING SAFE RENDER ---
const range = doc.bufferedPageRange();
for (let i = range.start; i < range.start + range.count; i++) {
  doc.switchToPage(i);
  // Draw footer line and text safely without invoking lineBreak page triggers
  doc.rect(40, 790, 515, 0.75).fill(C_BORDER);
  doc.fillColor(C_MUTED).fontSize(8).font('Helvetica');
  doc.text('Hospital Management System (HMS) - Technical & Functional Manual', 40, 798, {
    align: 'left',
    lineBreak: false,
  });
  doc.text(`Page ${i + 1} of ${range.count}`, 40, 798, {
    align: 'right',
    lineBreak: false,
  });
}
