...
const { LabTest, LabOrder, LabOrderItem, Patient, Doctor, User, PathologyParameter, MasterOption } = require('../../models');
...
const findAndCountTests = ({ filters = {}, dateRange, search, limit, offset }) => {
  ...
  return LabTest.findAndCountAll({ where, limit, offset, order: [['id', 'DESC']] });
};

const findTestById = (id, options = {}) => LabTest.findByPk(id, options);
const findTestByCode = (code, options = {}) => LabTest.findOne({ where: { code }, ...options });
const createTest = (data, options = {}) => LabTest.create(data, options);
...
const orderIncludes = [
  ...
  },
  {
    model: LabOrderItem,
    as: 'items',
    include: [{ model: LabTest, as: 'test', attributes: ['id', 'code', 'name', 'sample_type', 'unit'] }],
  },
];
...
const findAndCountOrders = ({ filters = {}, dateRange, search, limit, offset }) => {
  ...
  return LabOrder.findAndCountAll({
    where,
...
const findOrderById = (id, options = {}) => LabOrder.findByPk(id, { include: orderIncludes, ...options });
...
const createOrder = (data, options = {}) => LabOrder.create(data, options);
...
const createOrderItems = (items, options = {}) => LabOrderItem.bulkCreate(items, options);
const findOrderItemById = (id, options = {}) => LabOrderItem.findByPk(id, options);
const updateOrderItem = (item, changes, options = {}) => item.update(changes, options);

const parameterIncludes = [
  { model: MasterOption, as: 'unit_option', attributes: ['id', 'code', 'label'] },
];

const findAndCountPathologyParameters = ({ filters = {}, search, limit, offset }) => {
  const where = { ...filters };
  if (search) {
    where[Op.or] = [
      { name: { [Op.like]: `%${search}%` } },
  ...
  return PathologyParameter.findAndCountAll({
    where,
...
const findPathologyParameterById = (id, options = {}) =>
  PathologyParameter.findByPk(id, { include: parameterIncludes, ...options });
const createPathologyParameter = (data, options = {}) => PathologyParameter.create(data, options);
const updatePathologyParameter = (param, changes, options = {}) => param.update(changes, options);
const destroyPathologyParameter = (param, options = {}) => param.destroy(options);
...
module.exports = {
  ...
  findOrderItemById,
  updateOrderItem,
  findAndCountPathologyParameters,
  findPathologyParameterById,
  createPathologyParameter,
  updatePathologyParameter,
  destroyPathologyParameter,
};
